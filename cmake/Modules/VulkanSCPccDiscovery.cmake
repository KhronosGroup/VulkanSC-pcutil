...
function(resolve_vksc_pcc_executable PCC_EXECUTABLE PCC_JSON_PATH OUT_VAR)
    ...
    set(PCC_EXECUTABLE_PATH "")
    if(NOT PCC_EXECUTABLE MATCHES ${NATIVE_PATH_SEPARATOR_REGEX})
        separate_arguments(PCC_EXECUTABLE_ARGS NATIVE_COMMAND PROGRAM "${PCC_EXECUTABLE}")
        if(PCC_EXECUTABLE_ARGS)
            list(GET PCC_EXECUTABLE_ARGS 0 PCC_EXECUTABLE_CMD)
            if(EXISTS "${PCC_EXECUTABLE_CMD}")
                set(PCC_EXECUTABLE_PATH "${PCC_EXECUTABLE_CMD}")
            else()
                message(WARNING "PCC executable ('${PCC_EXECUTABLE}' resolved to '${PCC_EXECUTABLE_CMD}') not found (possibly misconfigured environment)")
            endif()
        else()
            message(WARNING "PCC executable ('${PCC_EXECUTABLE}') not found in path (possibly misconfigured environment)")
        endif()
    else()
        set(PCC_EXECUTABLE_PATH "${PCC_EXECUTABLE}")
        cmake_path(IS_RELATIVE PCC_EXECUTABLE_PATH PCC_EXECUTABLE_IS_RELATIVE)
        if(PCC_EXECUTABLE_IS_RELATIVE)
            cmake_path(GET PCC_JSON_PATH PARENT_PATH PCC_JSON_PARENT_PATH)
            cmake_path(ABSOLUTE_PATH PCC_EXECUTABLE_PATH BASE_DIRECTORY "${PCC_JSON_PARENT_PATH}" NORMALIZE)
        else()
            cmake_path(NORMAL_PATH PCC_EXECUTABLE_PATH)
        endif()
        if(NOT EXISTS "${PCC_EXECUTABLE_PATH}")
            message(WARNING "PCC executable ('${PCC_EXECUTABLE}' resolved to '${PCC_EXECUTABLE_PATH}') not found (possibly broken driver or vendor SDK installation)")
        endif()
    endif()
    set(${OUT_VAR} "${PCC_EXECUTABLE_PATH}" PARENT_SCOPE)
    message(DEBUG "PCC resolved: ${PCC_EXECUTABLE_PATH}")
endfunction()

# If a PCC executable is set manually but not a specific toolchain then do not perform the PCC discovery
if(NOT DEFINED VulkanSC_PCC_EXECUTABLE OR DEFINED VulkanSC_PCC)
            ...
            foreach(HIVE IN ITEMS HKCU HKLM)
                ...
                foreach(PCC_PROP_NAME IN LISTS PCC_PROP_NAMES)
                    ...
                    if(EXISTS "${PCC_PROP_NAME}")
                        ...
                        message(WARNING "Skipping PCC JSON manifest file '${PCC_PROP_NAME}' as it does not exist (possibly broken driver or vendor SDK installation)")
                    endif()
                endforeach()
            endforeach()
        else()
    ...
    foreach(PCC_JSON_PATH IN LISTS PCC_JSON_PATHS)
        ...
        if(NOT ARCH_LENGTH_ERROR)
            ...
            foreach(ARCH_INDEX RANGE ${ARCH_LENGTH_STOP})
                ...
                # Do not add entry if resolving the PCC executable failed
                if(NOT EXISTS "${PCC_EXECUTABLE}")
                    message(WARNING "PCC executable not found, skipping architecture: ${PCC_NAME} - ${ARCH_NAME} ('${PCC_JSON_PATH}')")
                    continue()
                endif()
            ...
            foreach(DEV_INDEX RANGE ${DEV_LENGTH_STOP})
                ...
                foreach(DEVICEFILTER_MATCH IN LISTS DEVICEFILTER_MATCHES)
                    if(DEVICEFILTER_MATCH MATCHES "${DEVICEFILTER_REGEX}")
                        ...
                        # Do not add entry if resolving the PCC executable failed
                        if(NOT EXISTS "${PCC_EXECUTABLE}")
                            message(WARNING "PCC executable not found, skipping device: ${PCC_NAME} - ${DEVICE_NAME} ('${PCC_JSON_PATH}')")
                            continue()
                        endif()
    ...
    if(VulkanSC_PCC)
        message(STATUS "Selected PCC toolchain: ${VulkanSC_PCC}")
    ...
    set(VulkanSC_PCC_EXECUTABLE "${VulkanSC_PCC_EXECUTABLE_DEFAULT}" CACHE FILEPATH "Path to the VulkanSC pipeline cache compiler" FORCE)
    set(VulkanSC_PCC_FLAGS "${VulkanSC_PCC_FLAGS_DEFAULT}" CACHE STRING "Flags used by the pipeline cache compiler" FORCE)
    set(VulkanSC_PCC_DEBUG_FLAG "${VulkanSC_PCC_DEBUG_FLAG_DEFAULT}" CACHE STRING "Flag used by the pipeline cache compiler to embed SPIR-V debug info" FORCE)
    set(VulkanSC_PCC_DEVICE_FLAGS "${VulkanSC_PCC_DEVICE_FLAGS_DEFAULT}" CACHE STRING "Flag used by the pipeline cache compiler to select targeting a specific device" FORCE)
    set(VulkanSC_PCC_VENDOR_ID_FILTER "${VulkanSC_PCC_VENDOR_ID_FILTER_DEFAULT}" CACHE STRING "The VK_LOADER_VENDOR_ID_FILTER given to the Vulkan SC loader" FORCE)
    set(VulkanSC_PCC_DEVICE_ID_FILTER "${VulkanSC_PCC_DEVICE_ID_FILTER_DEFAULT}" CACHE STRING "The VK_LOADER_DEVICE_ID_FILTER given to the Vulkan SC loader" FORCE)
    set(VulkanSC_PCC_DRIVER_ID_FILTER "${VulkanSC_PCC_DRIVER_ID_FILTER_DEFAULT}" CACHE STRING "The VK_LOADER_DRIVER_ID_FILTER given to the Vulkan SC loader" FORCE)
endif()
