...
if(NOT "${VulkanSC_PCC_EXECUTABLE}")
    ...
    foreach(PCC_JSON_PATH IN LISTS PCC_JSON_PATHS)
            ...
            foreach(ARCH_INDEX RANGE ${ARCH_LENGTH_STOP})
                ...
                if(NOT DEFINED VulkanSC_PCC_EXECUTABLE_${PCC_NAME}_${ARCH_NAME})
                    ...
                    if(NOT VENDOR_ID_FILTER_ERROR)
                        set(VulkanSC_VENDOR_ID_FILTER_${PCC_NAME}_${ARCH_NAME} ${VENDOR_ID_FILTER})
                    endif()
                    if(NOT DEVICE_ID_FILTER_ERROR)
                        set(VulkanSC_DEVICE_ID_FILTER_${PCC_NAME}_${ARCH_NAME} ${DEVICE_ID_FILTER})
                    endif()
                    if(NOT DRIVER_ID_FILTER_ERROR)
                        set(VulkanSC_DRIVER_ID_FILTER_${PCC_NAME}_${ARCH_NAME} ${DRIVER_ID_FILTER})
                    endif()
        ...
        if(NOT DEV_LENGTH_ERROR)
            ...
            foreach(DEV_INDEX RANGE ${DEV_LENGTH_STOP})
                ...
                if(NOT VENDOR_ID_FILTER_ERROR)
                    set(VENDOR_ARG "--vendor ${VENDOR_ID_FILTER}")
                endif()
                if(NOT DEVICE_ID_FILTER_ERROR)
                    set(DEVICE_ARG "--device ${DEVICE_ID_FILTER}")
                endif()
                if(NOT DRIVER_ID_FILTER_ERROR)
                    set(DRIVER_ARG "--driver ${DRIVER_ID_FILTER}")
                endif()
                ...
                message(DEBUG "DEVICEFILTER_COMMAND: ${DEVICEFILTER_COMMAND}")
                message(DEBUG "DEVICEFILTER_OUTPUT: ${DEVICEFILTER_OUTPUT}")
                ...
                foreach(DEVICEFILTER_MATCH IN LISTS DEVICEFILTER_MATCHES)
                    if(DEVICEFILTER_MATCH MATCHES "${DEVICEFILTER_REGEX}")
                        ...
                        if(NOT DEFINED VulkanSC_PCC_EXECUTABLE_${PCC_NAME}_${DEVICE_NAME})
                            ...
                            if(NOT VENDOR_ID_FILTER_ERROR)
                                set(VulkanSC_VENDOR_ID_FILTER_${PCC_NAME}_${DEVICE_NAME} ${VENDOR_ID_FILTER})
                            endif()
                            set(VulkanSC_DEVICE_ID_FILTER_${PCC_NAME}_${DEVICE_NAME} ${DEVICE_ID})
                            if(NOT DRIVER_ID_FILTER_ERROR)
                                set(VulkanSC_DRIVER_ID_FILTER_${PCC_NAME}_${DEVICE_NAME} ${DRIVER_ID_FILTER})
                            endif()
    ...
    if(NOT VulkanSC_PCC_OPTIONS_LENGTH EQUAL 0)
        ...
        message(WARNING "No installed Vulkan SC pipeline cache compiler found!")
    endif()
    ...
    if(VulkanSC_PCC)
        if(VulkanSC_PCC IN_LIST VulkanSC_PCC_OPTIONS)
            if(VulkanSC_PCC MATCHES "^([A-Za-z0-9]*) - ([^\n]*)$")
                ...
                set(VulkanSC_FLAGS_DEFAULT ${VulkanSC_FLAGS_${CMAKE_MATCH_1}_${CMAKE_MATCH_2}})
                set(VulkanSC_DEBUG_FLAG_DEFAULT ${VulkanSC_DEBUG_FLAG_${CMAKE_MATCH_1}_${CMAKE_MATCH_2}})
                set(VulkanSC_DEVICE_FLAGS_DEFAULT ${VulkanSC_DEVICE_FLAGS_${CMAKE_MATCH_1}_${CMAKE_MATCH_2}})
                set(VulkanSC_VENDOR_ID_FILTER_DEFAULT ${VulkanSC_VENDOR_ID_FILTER_${CMAKE_MATCH_1}_${CMAKE_MATCH_2}})
                set(VulkanSC_DEVICE_ID_FILTER_DEFAULT ${VulkanSC_DEVICE_ID_FILTER_${CMAKE_MATCH_1}_${CMAKE_MATCH_2}})
                set(VulkanSC_DRIVER_ID_FILTER_DEFAULT ${VulkanSC_DRIVER_ID_FILTER_${CMAKE_MATCH_1}_${CMAKE_MATCH_2}})
            else()
                message(FATAL_ERROR "Unexpected regex error while matching selected VulkanSC PCC pipeline cache compiler: ${VulkanSC_PCC}")
            endif()
        else()
            message(FATAL_ERROR "Not found any pipeline cache compiler configuration matching VulkanSC_PCC: ${VulkanSC_PCC}")
        endif()
    endif()

    set(VulkanSC_PCC_EXECUTABLE "${VulkanSC_PCC_EXECUTABLE_DEFAULT}" CACHE FILEPATH "Path to the VulkanSC pipeline cache compiler")
    set(VulkanSC_FLAGS "${VulkanSC_FLAGS_DEFAULT}" CACHE STRING "Flags used by the pipeline cache compiler")
    set(VulkanSC_DEBUG_FLAG "${VulkanSC_DEBUG_FLAG_DEFAULT}" CACHE STRING "Flag used by the pipeline cache compiler to embed SPIR-V debug info")
    set(VulkanSC_DEVICE_FLAGS "${VulkanSC_DEVICE_FLAGS_DEFAULT}" CACHE STRING "Flag used by the pipeline cache compiler to select targeting a specific device")
