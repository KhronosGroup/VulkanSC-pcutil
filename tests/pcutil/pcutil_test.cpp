...
static_assert(sizeof(VkPipelineCacheHeaderVersionOne) == 32, "VkPipelineCacheHeaderVersionOne has incorrect size");
...
static_assert(sizeof(VkPipelineCacheSafetyCriticalIndexEntry) == 56, "VkPipelineCacheSafetyCriticalIndexEntry has incorrect size");
...
#define PRINT_STRUCTSIZE(name) std::cout << "size of " #name ": " << sizeof(name) << std::endl;
#define PRINT_STRUCTMEMB(struc, memb) \
    std::cout << "{offset, size} of " #memb "\t\t: " << offsetof(struc, memb) << ", " << sizeof(struc().memb) << std::endl;
