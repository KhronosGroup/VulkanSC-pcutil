...
#define VC_EXTRALEAN
#endif
// Guard against redefinition of macro warning.
#ifndef NOMINMAX
#define NOMINMAX
#endif
#include <windows.h>
...
class PipelineData {
  public:
    ...
    ~PipelineData() {
        for (std::size_t i = 0; i < m_jsonStringsList.size(); i++) {
            delete (m_jsonStringsList[i]);
        }
        for (std::size_t i = 0; i < m_uuidAllocList.size(); i++) {
    ...
    VkResult createAndSetPrivateData(VkDevice& device, VkObjectType objectType, uint64_t objectHandle, std::string* data) {
        ...
        if (m_privateDataSlot == nullptr) {
            ...
            if (m_usePrivateDataExtension) {
                result = device_dispatch_table(device)->CreatePrivateDataSlotEXT(device, &pPrivateDataSlotCreateInfo, NULL,
                                                                                 &m_privateDataSlot);
            } else {
                result = device_dispatch_table(device)->CreatePrivateDataSlot(device, &pPrivateDataSlotCreateInfo, NULL,
                                                                              &m_privateDataSlot);
            }
        ...
        if (m_usePrivateDataExtension) {
            result = device_dispatch_table(device)->SetPrivateDataEXT(device, objectType, objectHandle, m_privateDataSlot,
                                                                      reinterpret_cast<uint64_t>(data));
        } else {
            result = device_dispatch_table(device)->SetPrivateData(device, objectType, objectHandle, m_privateDataSlot,
                                                                   reinterpret_cast<uint64_t>(data));
        }
    ...
    void setPipelineLayout(VkDevice& device, const VkPipelineLayoutCreateInfo* pCreateInfo,
    ...
    void deleteShaderModuleInfo(VkShaderModule m) { m_shaderInfoMap.erase(m); }
    ...
    VkResult setPipelineUUID(VkDevice& device, VkPipeline pPipeline, uint8_t uuid[VK_UUID_SIZE]) {
        ...
        if (m_usePrivateDataExtension) {
            result = device_dispatch_table(device)->SetPrivateDataEXT(device, VK_OBJECT_TYPE_PIPELINE,
                                                                      reinterpret_cast<uint64_t>(pPipeline), m_privateDataUuidSlot,
                                                                      reinterpret_cast<uint64_t>(uuidMem));
        } else {
            result = device_dispatch_table(device)->SetPrivateData(device, VK_OBJECT_TYPE_PIPELINE,
                                                                   reinterpret_cast<uint64_t>(pPipeline), m_privateDataUuidSlot,
                                                                   reinterpret_cast<uint64_t>(uuidMem));
        }
    ...
    void createUUID(const std::string& hashString, uint8_t uuid[VK_UUID_SIZE]) {
        hashing::md5::hash_bs(hashString.c_str(), hashString.length(), uuid);
    ...
    void dumpComputePipeline(VkDevice& device, uint32_t createInfoCount, const VkComputePipelineCreateInfo* pCreateInfos,
        ...
        for (uint32_t cnt = 0; cnt < createInfoCount; cnt++) {
            ...
            std::string* pipelineLayoutCreateInfoString =
                getPrivateData(device, VK_OBJECT_TYPE_PIPELINE_LAYOUT, reinterpret_cast<uint64_t>(pCreateInfos[cnt].layout));
    ...
    void dumpImmutableSamplers() {
        ...
        for (const auto& keyVal : m_ImmutableSamplerBindingMap) {
            ...
            for (int i = 0; i < vk_json::s_num_spaces; i++) vk_json::_string_stream << " ";
            vk_json::_string_stream << "\""
                                    << "\"pImmutableSamplers_" << m_ImmutableSamplerBindingMap[keyVal.first]
                                    << "\" : " << std::endl;
    ...
    void dumpDescriptorSets(VkDevice& device, VkPipelineLayout layout, std::vector<std::string>& pLayoutsHashStringList) {
        ...
        for (auto& it : layouts) {
            ...
            std::string* descriptorSetLayoutCreateInfoString =
                getPrivateData(device, VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, reinterpret_cast<uint64_t>(it));
    ...
    std::stringstream dumpShaders(uint32_t stageCount, const VkPipelineShaderStageCreateInfo* pStages) {
        ...
        for (unsigned int i = 0; i < stageCount; i++) {
            ...
            for (std::size_t k = 0; k < m_shaderInfoMap[pStages[i].module].m_codeSize; k++) {
                shaderCode << *p;
    ...
    void dumpGraphicsPipeline(VkDevice& device, uint32_t createInfoCount, const VkGraphicsPipelineCreateInfo* pCreateInfos,
                              VkPipeline* pPipelines) {
        for (uint32_t cnt = 0; cnt < createInfoCount; cnt++) {
            FILE* fp = openFile();
            ...
            std::string* renderPassCreateInfoString =
                getPrivateData(device, VK_OBJECT_TYPE_RENDER_PASS, reinterpret_cast<uint64_t>(pCreateInfos[cnt].renderPass));
    ...
    void dumpObjectReservationInfo() {
        ...
        ss << "\tg_objectResCreateInfo.sType                                      = "
        ...
        ss << "\tg_objectResCreateInfo.descriptorSetLayoutBindingRequestCount     = "
           << objResInfo.descriptorSetLayoutBindingRequestCount << ";\n";
...
void set_layer_supported_features(VkPhysicalDeviceFeatures2* pFeatures) {
    ...
    while (pNext != nullptr) {
        if (pNext->sType == VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PIPELINE_PROPERTIES_FEATURES_EXT) {
            VkPhysicalDevicePipelinePropertiesFeaturesEXT* features =
                reinterpret_cast<VkPhysicalDevicePipelinePropertiesFeaturesEXT*>(pNext);
...
EXPORT_FUNCTION VKAPI_ATTR VkResult VKAPI_CALL vkGetPipelinePropertiesEXT(VkDevice device, const VkPipelineInfoEXT* pPipelineInfo,
