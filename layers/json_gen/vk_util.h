...
#define vk_foreach_struct(name, first) for (VkBaseOutStructure *name = first; name != NULL; name = name->pNext)

#define vk_foreach_struct_const(name, first) for (const VkBaseInStructure *name = first; name != NULL; name = name->pNext)

static inline const void *_vk_find_struct_const(const VkBaseInStructure *chain, VkStructureType sType) {
    while (chain) {
        if (chain->sType == sType) return chain;
        chain = chain->pNext;
    }
    return NULL;
}

static inline void *_vk_find_struct(VkBaseOutStructure *chain, VkStructureType sType) {
    while (chain) {
        if (chain->sType == sType) return chain;
        chain = chain->pNext;
    }
...
#define vk_find_struct_const(chain, type) _vk_find_struct_const(chain, VK_STRUCTURE_TYPE_##type)

#define vk_find_struct(chain, type) _vk_find_struct(chain, VK_STRUCTURE_TYPE_##type)
...
static inline void _vk_outarray_init(struct _vk_outarray *a, void *data, uint32_t *restrict len) {
    a->data = data;
    a->cap = *len;
    a->filled_len = len;
    *a->filled_len = 0;
...
static inline VkResult _vk_outarray_status(const struct _vk_outarray *a) {
...
static inline void *_vk_outarray_next(struct _vk_outarray *a, size_t elem_size) {
    void *p = NULL;
...
#define vk_outarray_init(a, data, len) _vk_outarray_init(&(a)->base, (data), (len))
...
#define vk_outarray_status(a) _vk_outarray_status(&(a)->base)

#define vk_outarray_next(a) ((vk_outarray_typeof_elem(a) *)_vk_outarray_next(&(a)->base, vk_outarray_sizeof_elem(a)))
 ...
 */
#define vk_outarray_append(a, elem) for (vk_outarray_typeof_elem(a) *elem = vk_outarray_next(a); elem != NULL; elem = NULL)
...
#define VK_MULTIALLOC(_name) struct vk_multialloc _name = VK_MULTIALLOC_INIT

static inline uint64_t align_u64(uint64_t v, uint64_t a) {
...
#define vk_multialloc_add_size(_ma, _ptr, _size) _vk_multialloc_add((_ma), (void **)(_ptr), (_size), __alignof__(**(_ptr)))

#define vk_multialloc_add(_ma, _ptr, _count) vk_multialloc_add_size(_ma, _ptr, (_count) * sizeof(**(_ptr)));
...
#define _VK_MULTIALLOC_UPDATE_POINTER(_i) \
    if ((_i) < ma->ptr_count) *ma->ptrs[_i] = (void *)((uintptr_t)ptr + (uintptr_t)*ma->ptrs[_i])
