 ...
 * @param output_sig Pointer to the 128-bit signature
 */
void hash_bs(const void* input_bs, uint64_t input_size, void* output_sig) {
    ...
    std::array<uint32_t, 64> s = {7,  12, 17, 22, 7,  12, 17, 22, 7,  12, 17, 22, 7,  12, 17, 22, 5,  9,  14, 20, 5,  9,
     ...
     * @endcode
     */
    std::array<uint32_t, 64> K = {3614090360, 3905402710, 606105819,  3250441966, 4118548399, 1200080426, 2821735955, 4249261313,
    ...
    auto* sig = static_cast<uint8_t*>(output_sig);  // new uint8_t[16];
